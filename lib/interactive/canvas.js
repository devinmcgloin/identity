const resize = (canvas, onResize) => {
  ...
  if (canvas.width !== displayWidth || canvas.height !== displayHeight) {
    ...
    if (onResize)
      onResize(canvas.getContext('2d'), displayWidth, displayHeight);
  }
...
const updateCanvas = (canvas, context, draw, onResize, clear) => {
...
const setupCanvas = (
  ...
  if (typeof window !== 'undefined') {
    setInterval(
      () => updateCanvas(canvas, context, draw, onResize, clear),
      refreshRate
    );
  }
  setup(context, canvas.width, canvas.height);
};
